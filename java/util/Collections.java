 ...
 *
 *  static <T> Queue<T> asLifoQueue(Deque<T> deque) 将一个双端队列转换成一个先进先出的队列，然后返回先进先出队列
 *
 *  static <T> int binarySearch(List<? extends Comparable<? super T>> list, T key) 使用二叉搜索算法搜索指定对象的指定列表。
 *  指定列表需要是有序的列表，当没有找到时，返回的结果不一定是-1。详细说明：https://blog.csdn.net/weixin_41463193/article/details/88653065
 *
 *  static <T> int binarySearch(List<? extends T> list, T key, Comparator<? super T> c) 使用二叉搜索算法搜索指定对象的指定列表。
 *  同上
 *
 *  static <E> Collection<E> checkedCollection(Collection<E> c, Class<E> type) 方法用于获取指定集合的一个动态类型安全视图。
 *
 *  static <E> List<E> checkedList(List<E> list, Class<E> type) 方法用于获取指定列表的一个动态类型安全视图。
 *
 *  static <K,V> Map<K,V> checkedMap(Map<K,V> m, Class<K> keyType, Class<V> valueType) 方法用于获得指定映射的一个动态类型安全视图。
 *
 *  static <K,V> NavigableMap<K,V> checkedNavigableMap(NavigableMap<K,V> m, Class<K> keyType, Class<V> valueType)
 *  返回指定NavigableMap的动态类型安全视图。
 *
 *  static <E> NavigableSet<E> checkedNavigableSet(NavigableSet<E> s, Class<E> type)
 *  返回指定的NavigableSet的动态类型安全视图。
 *
 *  static <E> Queue<E> checkedQueue(Queue<E> queue, Class<E> type)
 *  返回指定队列的动态类型安全视图。
 *
 *  static <E> Set<E> checkedSet(Set<E> s, Class<E> type) 方法用于获取指定Set的动态类型安全视图。
 *
 *  static <K,V> SortedMap<K,V> checkedSortedMap(SortedMap<K,V> m, Class<K> keyType, Class<V> valueType)
 *  方法用于获取指定有序映射的一个动态类型安全视图。
 *
 *  static <E> SortedSet<E> checkedSortedSet(SortedSet<E> s, Class<E> type) 方法用于获取指定有序set的一个动态类型安全视图。
 *
 *  static <T> void copy(List<? super T> dest, List<? extends T> src) 方法用于将所有从一个列表中的元素复制到另一个。
 *  dest是目标对象，src是源对象
 *
 *  static boolean disjoint(Collection<?> c1, Collection<?> c2) 方法用于为'true'，如果两个指定collection中没有相同的元素。
 *
 *  static <T> Enumeration<T> emptyEnumeration() 返回没有元素的枚举。
 *
 *  static <T> Iterator<T> emptyIterator() 返回没有元素的迭代器。
 *
 *  static <T> List<T> emptyList() 返回空列表（不可变）。
 *
 *  static <T> ListIterator<T> emptyListIterator() 返回没有元素的列表迭代器。
 *
 *  static <K,V> Map<K,V> emptyMap() 返回空的地图（不可变）。
 *
 *  static <K,V> NavigableMap<K,V> emptyNavigableMap() 返回空导航Map（不可变）。
 *
 *  static <E> NavigableSet<E> emptyNavigableSet() 返回一个空导航Set（不可变）。
 *
 *  static <T> Set<T> emptySet() 返回一个空集（不可变）。
 *
 *  static <K,V> SortedMap<K,V> emptySortedMap() 返回空的排序Map（不可变）。
 *
 *  static <E> SortedSet<E> emptySortedSet() 返回一个空的排序Set（不可变）。
 *
 *  static <T> Enumeration<T> enumeration(Collection<T> c) 返回指定集合的枚举。
 *
 *  static <T> void fill(List<? super T> list, T obj) 用指定的元素代替指定列表的所有元素。
 *
 *  static int frequency(Collection<?> c, Object o) 返回指定集合中与指定对象出现的次数。
 *
 ...
 *
 *  static <T extends Object & Comparable<? super T>> T min(Collection<? extends T> coll) 根据其元素的自然顺序返回给定集合的最小元素。
 *
 *  static <T> T min(Collection<? extends T> coll, Comparator<? super T> comp) 根据指定的比较器引发的顺序返回给定集合的最小元素。
 *
 *  static <T> List<T> nCopies(int n, T o) 返回由指定对象的n副本组成的不可变列表。
 *
 ...
 *
 *  static <K,V> Map<K,V> singletonMap(K key, V value) 返回一个不可变的Map，只将指定的键映射到指定的值。
 *
 ...
 *
 *  static <K,V> NavigableMap<K,V> synchronizedNavigableMap(NavigableMap<K,V> m) 返回由指定的NavigableMap支持的同步（线程安全）NavigableMap。
 *
 *  static <T> NavigableSet<T> synchronizedNavigableSet(NavigableSet<T> s) 返回由指定的NavigableSet支持的同步（线程安全）NavigableSet。
 *
 ...
 *
 *  static <K,V> Map<K,V> unmodifiableMap(Map<? extends K,? extends V> m) 返回指定Map的不可修改视图。
 *
 *  static <K,V> NavigableMap<K,V> unmodifiableNavigableMap(NavigableMap<K,? extends V> m) 返回指定NavigableMap的不可修改视图。
 *
